...
f, ax, p = PlantGeom.plantviz(meshes)
save("reference_images/refmesh_basic.png", f)

f, ax, p = plantviz(meshes, color=Dict("Mesh0" => :burlywood4, "Mesh1" => :springgreen4))
save("reference_images/refmesh_allcolors.png", f)

# Or just changing the color of some:
f, ax, p = plantviz(meshes, color=Dict("Mesh1" => :burlywood4))
...
f, ax, p = plantviz(opf, color=:red)
save("reference_images/opf_one_color.png", f)

# One color per reference mesh:
f, ax, p = plantviz(opf, color=Dict("Mesh0" => :burlywood4, "Mesh1" => :springgreen4))
save("reference_images/opf_one_color_per_ref.png", f)

# Or just changing the color of some:
f, ax, p = plantviz(opf, color=Dict("Mesh1" => :burlywood4))
...
f, ax, p = plantviz(opf, color=Dict("Mesh0" => vertex_color))
...
f, ax, p = plantviz(opf, color=:z_max)
...
f, ax, p = plantviz(opf, color=:z, showsegments=true)
save("reference_images/opf_color_attribute_vertex.png", f)

f, ax, p = plantviz(opf, color=:z)
colorbar(f[1, 2], p)
save("reference_images/opf_color_attribute_colorbar.png", f)

f, ax, p = plantviz(opf, color=:z, colorrange=(0u"m", 50u"m"))
colorbar(f[1, 2], p)
save("reference_images/opf_color_attribute_colorbar_range.png", f)

f, ax, p = plantviz(opf, color=:Length, colorrange=(0, 0.2), color_cache_name=:test_color)
leaf = get_node(opf, 5)
