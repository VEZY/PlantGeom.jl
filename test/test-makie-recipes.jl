...
@testset "Makie recipes: reference meshes -> image references" begin
    ...
    viz(fig[1, 1], meshes)
    @test_reference "reference_images/refmesh_basic.png" fig
    fig = Figure(size=(600, 800))
    viz(fig[1, 1], meshes, color=Dict(1 => :burlywood4, 2 => :springgreen4))
    @test_reference "reference_images/refmesh_allcolors.png" fig
    fig = Figure(size=(600, 800))
    viz(fig[1, 1], meshes, color=Dict(2 => :burlywood4))
    @test_reference "reference_images/refmesh_somecolors.png" fig
...
@testset "Makie recipes: whole MTG -> image references" begin
    ...
    fig = Figure(size=(600, 800))
    viz(fig[1, 1], opf, color=:red)
    @test_reference "reference_images/opf_one_color.png" fig
    fig = Figure(size=(600, 800))
    viz(fig[1, 1], opf, color=Dict(1 => :burlywood4, 2 => :springgreen4))
    @test_reference "reference_images/opf_one_color_per_ref.png" fig
    fig = Figure(size=(600, 800))
    viz(fig[1, 1], opf, color=Dict(1 => :burlywood4))
    @test_reference "reference_images/opf_one_color_one_ref.png" fig
    vertex_color = get_color(1:nvertices(get_ref_meshes(opf))[1], [1, nvertices(get_ref_meshes(opf))[1]])
    fig = Figure(size=(600, 800))
    viz(fig[1, 1], opf, color=Dict(1 => vertex_color), color_vertex=true)
    @test_reference "reference_images/opf_color_ref_vertex.png" fig
    transform!(opf, zmax => :z_max, ignore_nothing=true)
    fig = Figure(size=(600, 800))
    viz(fig[1, 1], opf, color=:z_max)
    @test_reference "reference_images/opf_color_attribute.png" fig

    transform!(opf, :geometry => (x -> [i.coords[3] for i in x.mesh.vertices]) => :z, ignore_nothing=true)
    fig = Figure(size=(600, 800))
    viz(fig[1, 1], opf, color=:z, showfacets=true, color_vertex=true)
    @test_reference "reference_images/opf_color_attribute_vertex.png" fig

    fig = Figure(size=(600, 800))
    ax, p = viz(fig[1, 1], opf, color=:z)
    colorbar(fig[1, 2], p)
    @test_reference "reference_images/opf_color_attribute_colorbar.png" fig

    fig = Figure(size=(600, 800))
    ax, p = viz(fig[1, 1], color=:z, color_range=(0, 50))
    colorbar(fig[1, 2], p)
...
@testset "Makie recipes: observables, change colorscale range" begin
    fig = Figure(size=(600, 800))
    ax, p = viz(fig[1, 1], color=:Length, color_range=(0, 0.2))
