...
function tree_demo_prototypes()
    ...
    Dict(
        :Internode => RefMeshPrototype(stem_ref; warn=false),
        :Leaf => RefMeshPrototype(leaf_ref; warn=false),
    )
end
...
function _apply_segment_coordinates!(node, p0, p1)
...
function add_axis_segment!(parent, edge, idx, p0, p1, radius)
    node = emit_internode!(
        parent;
        index=idx,
        link=edge,
    ...
    _apply_segment_coordinates!(node, p0, p1)
    node
end

function add_leaf_segment!(parent, idx, anchor, azimuth_deg, tilt_deg, leaf_length)
    tip = advance_point(anchor, azimuth_deg, tilt_deg, leaf_length)
    node = emit_leaf!(
        parent;
        index=idx,
        length=leaf_length,
        width=0.36 * leaf_length,
        thickness=0.04 * leaf_length,
        bump_scene=false,
    )
    _apply_segment_coordinates!(node, anchor, tip)
    node
end
...
function grow_axis_chain!(parent, start_point, axis_counter;
    ...
    for seg in 1:n_segments
        ...
        node = add_axis_segment!(node, edge, axis_counter[], p0, p1, seg_radius)
...
function add_leaf_fan!(carrier, leaf_counter; n=2, azimuth_shift=84.0, fan_step=9.0, tilt=54.0)
    n == 0 && return
    for k in 1:n
        t = 0.42 + 0.42 * (k / (n + 1))
...
function build_demo_tree_with_growth_api()
    ...
    prototypes = tree_demo_prototypes()
    rebuild_geometry!(
        tree,
        prototypes;
        amap_options=AmapReconstructionOptions(explicit_coordinate_mode=:explicit_start_end_required),
        root_align=false,
        bump_scene=false,
    )
    tree
